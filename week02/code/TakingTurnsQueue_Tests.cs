...
public class TakingTurnsQueueTests
{
    ...
    // Run 10 times.
    // Expected Result: Bob, Tim, Sue, Bob, Tim, Sue, Tim, Sue, Tim, Tim
    // Defect(s) Found: Person with turns=0 (infinite) was not re-enqueued because the code
    // only checked turns > 1, missing the infinite turns case (turns <= 0). Fixed by adding
    // a separate check for turns <= 0 to re-enqueue without modifying the turns value.
    ...
    // Defect(s) Found: Person with turns=-3 (infinite) was not re-enqueued because the code
    // only checked turns > 1, missing the infinite turns case (turns <= 0). Fixed by adding
    // a separate check for turns <= 0 to re-enqueue without modifying the turns value.
